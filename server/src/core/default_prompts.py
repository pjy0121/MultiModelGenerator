"""
기본 레이어 프롬프트 상수들
React 클라이언트의 defaultPrompts.ts를 Python 버전으로 변환
"""

# Layer별 기본 프롬프트 설정
DEFAULT_LAYER_PROMPTS = {
    "generation": """당신은 요구사항 분석 전문가입니다. 지식 베이스 컨텍스트로부터 키워드 '{layer_input}'에 대한 상세하고 구체적인 요구사항을 최대한 많이 생성해주세요.
...
3. **반드시 컨텍스트 내용 안에서만 요구사항을 도출하세요** - 일반적인 지식이나 추측 금지
4. 컨텍스트에 명시된 내용에서 과하게 변형하지 마세요
5. **근거(reference)는 반드시 제공된 컨텍스트에서 직접 인용하세요**
6. **근거에는 원문의 정확한 문장과 페이지 번호(ex: Page 123) 또는 섹션 번호를 포함하세요**
7. 요구사항 ID는 REQ-001부터 순차적으로 부여하세요
8. 요구사항은 영어로 작성하세요
9. **근거 형식: "원문 인용문" [Page X] 또는 "원문 인용문" [Section Y.Z]**
...
{{
  ...
  "forward_data": "| ID | 요구사항 (Requirement) | 근거(reference) |\\n|---|---|---|\\n| REQ-001 | [구체적인 요구사항 내용] | \\\"[컨텍스트에서 직접 인용한 원문]\\\" [Page/Section 정보] |\\n| REQ-002 | ... | ... |"
}}
```
...
5. forward_data의 표에서 \\n은 실제 줄바꿈을 의미합니다""",
    ...
    "ensemble": """당신은 여러 지식을 통합하는 기술의 전문가입니다. 다음 여러 주어진 요구사항 표들을 하나로 통합하고 일관된 형식으로 정리하세요.
...
6. 내용 수정 및 항목 제거 없이 검증 상태만 변경하세요""",
    ...
    "validation": """당신은 기술 문서 검증 전문가입니다. 주어진 요구사항 표에서 검증 상태가 "unchecked" 상태인 요구사항들을 지식 베이스 컨텍스트를 바탕으로 순차적으로 검증하고 검증 상태를 변경해주세요.
...
6. **내용 변경 금지**: 요구사항의 내용(Requirement)과 근거(reference)는 절대 변경하지 마세요. 검증 상태(검증 상태 컬럼)만 변경하세요
...
6. 요구사항 내용과 근거(reference)는 절대 변경하지 마세요. 오직 검증 상태만 변경하세요"""
}

# 검색 강도별 top_k 값 매핑
SEARCH_INTENSITY_MAPPING = {
    "low": 30,
    "medium": 50,
    "high": 100
}